...
class DetectionWorker:
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                zdet = self._process_zoom(zjob)
                if zdet:
                    with self._lock:
                        self._zoom_results.append(zdet)
            except queue.Empty:
                pass
    ...
    def _process_zoom(self, job: _ZoomJob) -> Optional[Detection]:
        ...
        for det in ai_results:
            ox1 = int(rx1 + det.x1 * sx)
            oy1 = int(ry1 + det.y1 * sy)
            ox2 = int(rx1 + det.x2 * sx)
            oy2 = int(ry1 + det.y2 * sy)
...
class FusionEngine:
    ...
    @staticmethod
    def _make_debug(ai_count: int, cv_count: int, matches: int, fused_count: int, confirmed_count: int, possible_count: int) -> Dict:
        return {
            "ai_count": ai_count,
            "cv_count": cv_count,
            "matches": matches,
    ...
    def _fuse_decision(self, ai_det: Optional[Detection], cv_det: Optional[Detection], frame: np.ndarray) -> Optional[Detection]:
    ...
    def _update_tracking(self, fused_dets: List[Detection]) -> List[TrackedObject]:
        ...
        if self.detect_count % CLEANUP_INTERVAL == 0:
            self.tracked_objects = {oid: obj for oid, obj in self.tracked_objects.items() if obj.is_active}
    ...
    def draw_annotations(
        ...
        for det in ai_dets:
            ...
            cv2.putText(out, f"AI:{det.confidence:.2f}", (det.x1, det.y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.35, config.COLOR_AI, 1)

        for det in cv_dets:
            cv2.rectangle(out, (det.x1, det.y1), (det.x2, det.y2), config.COLOR_CV, 1)
            cv2.putText(out, f"CV:{det.confidence:.2f}", (det.x1, det.y2 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.35, config.COLOR_CV, 1)

        for obj in confirmed:
            det = obj.detection
            color = (0, 165, 255) if obj.id == target_id else config.COLOR_FUSED
            cv2.rectangle(out, (det.x1, det.y1), (det.x2, det.y2), color, 2)
            cv2.putText(out, f"#{obj.id} {det.confidence:.2f}", (det.x1, det.y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        ...
        for obj in possible:
            ...
            cv2.putText(out, f"P#{obj.id} {display_score:.2f}", (det.x1, det.y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, config.COLOR_POSSIBLE, 1)

        cv2.circle(out, (gripper_x, gripper_y), 8, (255, 0, 255), -1)
        cv2.putText(out, "GRIPPER", (gripper_x + 10, gripper_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)

        if target_center is not None:
            cv2.line(out, (gripper_x, gripper_y), target_center, (0, 165, 255), 2)

        cv2.putText(out, f"Frame {frame_count} | Hits: {len(confirmed)} | Possible: {len(possible)}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        cv2.putText(out, f"Robot: {movement_text}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
        return out

    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, List[TrackedObject], Dict, Optional[np.ndarray]]:
        self.frame_count += 1

        small = cv2.resize(frame, (0, 0), fx=INFER_SCALE, fy=INFER_SCALE, interpolation=cv2.INTER_LINEAR)
        ...
        if not target_pool and config.POSSIBLE_TARGET_FALLBACK_ENABLED:
            ...
            def possible_score(p: TrackedObject) -> float:
                ...
                if self._is_cv_like(src):
                    return det_conf
                if self._is_ai_like(src):
                    return det_conf * config.POSSIBLE_AI_CONF_WEIGHT
